...
open(PARAM,'diffexparams.txt') or die;
chdir($statsOutDir);
my ($genome,$genes,$cpu);
...
while(<PARAM>) {
    ...
    close GUIDE;
    system("collateSTARSTARusingGuideFile2.pl $outdir/guide_file > $outdir/merged_counts.txt");
    ...
    my $outfile = $grp1name . 'VS' . $grp2name . '.deseq2starstar.results.txt';
